...
CvMemStorage* memstorage = NULL;
 ...
 int main() {
	...
	*/
	...
	//cvRectangle(original,cvPoint(cent,cent1),cvPoint(300,300),CV_RGB(0,255,0),5,8);
	cvCircle(original,cvPoint(cent,cent1),50,CV_RGB(0,255,0),-1);
	cvShowImage( "Red extract", original );

		cout <<"Measurement of contours\n"
			<<"area:" << Area<<"\n"
			<< "length:"<<length<<"\n"
			<<"center of contours:"<<cent<<","
			<<cent1;
